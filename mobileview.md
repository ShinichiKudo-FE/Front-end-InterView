...
具体的canvas API 参见：JavaScript 标准参考教程（alpha）
...
zeptojs内置Touch events方法，具体可以看the aerogel-weight jQuery-compatible JavaScript library events
...
Android3+和iOS5+支持CSS3的新属性为`overflow-scrolling`
...
除非你先使用`apple-mobile-web-app-capable`指定全屏模式，否则这个meta标签不会起任何作用。
